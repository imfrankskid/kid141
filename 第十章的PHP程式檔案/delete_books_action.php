...
<html>
<head>
<title>完成刪除 Books 關聯表中指定的資料</title>
</head>
<body bgcolor="#ffffcc" alink=green vlink=blue>
<?php
   //產生一個delete指令
   $dsql="delete from Books where " . $_SESSION["delcond"];
   echo "<br>所產生的 SQL 指令為：" . $dsql;
   echo "<table border=0><tr><td width=900>";
   echo "<td><input type='button' value='回刪除網頁' OnClick=location.href('delete_books.php')>";
   echo "<td><input type='button' value='回首頁' OnClick=location.href('default.php')>";
   echo "</table>";
   //執行一個刪除指令
   if ($conn->query($dsql)) {     
      echo "<font size=4 color=brown><b>成功刪除 Books 中的 " . $_SESSION["delno"] . " 筆資料！";
      if ($_REQUEST["infrno"]>0)
      echo "連帶刪除 Orders 中的 " . $_REQUEST["infrno"] . " 筆資料！";
      echo "<center>";
      echo "<h2><font color=green>資料刪除後關聯表的內容<hr width=400>";
      echo "<table border=0>";
      echo "<tr valign=top>";
      ...
      echo "</table>";
      if ($_REQUEST["infrno"]>0) {  //表示連鎖反應，一併刪除Orders資料
   ...
   {
      echo "<font size=4 color=brown><b>刪除失敗！</b><br>";
      echo "<font size=3 color=green>因為外來鍵的刪除規則為<font color=brown>RESTRICTED</font>，";
      echo "且此筆資料被關聯表Orders的資料參考到！";
   }
?>
</body>
</html>
